...
Status: Started for runtime RmlUi UI foundation, package scripting/profile UI, editor-owned runtime config packaging, and code-owned runtime config fallbacks. Animation, audio, full UI polish, embedded package runtime loading, and packaged app smoke coverage remain incomplete.
...
- [x] Provide code-owned runtime config fallbacks so packaged client/server/editor reset paths do not depend on editable config files for defaults.
...
- [ ] Package profiles, package command construction, `File > Package...`, Windows/Linux package scripts, and `just package-client` / `just package-server` exist. Current packages stage the app executable, sibling TOML copied from `data/config/editor.toml`, and app-local dynamic library dependencies, and generate `.hkpack` resource bundles; app targets now have code-owned config fallbacks, but they do not embed resource bundles yet and runtime UI/scene/tuning/asset/shader reads still use filesystem paths.
